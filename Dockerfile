...
RUN apt-get update -qq && DEBIAN_FRONTEND=noninteractive apt-get -q -y install libmysqlclient-dev git curl software-properties-common python-software-properties
...
RUN echo "    database_host: db" >> app/config/parameters.yml.dist \
    ...
    && echo "    database_password: 98MeAjsGtGrdu4U9nKVcDQ" >> app/config/parameters.yml.dist
